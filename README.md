...
| `rusm-logfmt` | lib | **The shared log format** — the single source for the platform log's palette, column widths, timestamp, and tty-gated colour, so `rusm-otp`'s lifecycle lines and downstream app logs read as one aligned stream. Pure (std only); compiles for the host and `wasm32-wasip2`. **No `wasmtime` dependency.** |
