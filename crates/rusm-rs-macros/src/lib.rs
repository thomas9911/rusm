...
/// The `rusm:runtime` WIT — the single source shared with the `rusm-rs` crate, so a
/// component generated by [`main`] is bindings-identical to one with a hand-written
/// `wit/` dir. Embedded `inline:` into `generate!` so the component carries no `wit/`.
const RUNTIME_WIT: &str = include_str!("../../rusm-rs/wit/world.wit");
