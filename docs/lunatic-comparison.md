...
| 11 ⏳ | **Serving** (HTTP / WS / SSE from a component) | ✅ engine built+measured, **from both Rust and TS guests** — `http_server` (instance-per-request `wasi:http`, ~64.5k req/s), `ws_server` (one sandboxed component process per WS connection, ~192k echo round-trips/s), SSE streaming body (~1.5M events/s); 6 live dashboard scenarios (http/ws/sse × RS/TS) + connection-scale. Remaining: `rusm serve` + `rusm.toml [[http]]`, serving TLS | ❌ **no `wasi:http` host** (core modules only) | 🔥 **ahead — an axis Lunatic lacks** |
| — | SQLite host API | ❌ | 🅛 | — n/a |
