...
| 10 | **Scale & hardening** — *not raw speed* (throughput/latency is already at the isolation-model ceiling: ~440k component spawns/s, ~21M msgs/s). Shipped: **on-demand instance tier ✅** (`WasmRuntime::with_overflow` — when the pooled cap is full, spawn from an on-demand engine so the live *Wasm*-process count is bounded by **available memory**, not a compile-time pool size; the OTP core already runs millions of *native* processes — this lifts the artificial cap for *Wasm*-backed ones); **opt-in bounded mailboxes ✅** (`Runtime::with_mailbox_capacity` — shed *user* messages past capacity, system signals never shed); **cluster security hardening ✅** (`ClusterCa` issues per-node certs, mutual TLS, foreign-CA peers rejected — replacing Phase 9's pre-shared cert). Remaining: **supervisor restart-intensity** (max restarts per time window, in the in-guest supervisors). See the [design analysis](./design-analysis.md). | — |
