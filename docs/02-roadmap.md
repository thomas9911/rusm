...
| 8 🔨 | **Guest ergonomics** — **`rusm-ts`** ✅ (TS/Bun → the rquickjs **js-runner** giving JS the actor `Process` API, no jco): `rusm build` bundles a TS component with Bun → `wasm/<name>.js`, the app-model loader runs it on the shared runner under its capability profile, `rusm.d.ts` types the `Process` API, and binary messages + byte streams work from JS. Remaining: **`rusm-rs`** (Rust guest crate: spawn/Mailbox/AbstractProcess/Supervisor) — not started. | — |
