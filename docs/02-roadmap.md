...
| 10 | **Scale & hardening** — *not raw speed* (throughput/latency is already at the isolation-model ceiling: ~440k component spawns/s, ~21M msgs/s). An **on-demand instance tier** above the pooled ceiling (overflow instances allocate on demand — the path to millions of live *Wasm* processes), **opt-in bounded mailboxes** (overload back-pressure / load-shed), and **supervisor restart-intensity**. See the [design analysis](./design-analysis.md). | — |
