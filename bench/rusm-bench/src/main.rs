...
use rusm_bench::{serve, summarize_frame, Node, Runner, RunnerConfig, Scenario};
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => match args.get(2).and_then(|id| Scenario::from_id(id)) {
            Some(scenario) => {
                ...
                run_terminal(scenario, seconds).await;
            }
            None => {
                eprintln!("usage: rusm-bench run <scenario> [seconds]");
                eprintln!("scenarios: {}", Scenario::ALL.map(|s| s.id()).join(", "));
                std::process::exit(2);
            }
        },
        _ => {
            eprintln!("usage: rusm-bench <serve [addr] | run <scenario> [seconds]>");
            std::process::exit(2);
        }
    }
}

async fn run_terminal(scenario: Scenario, seconds: u64) {
